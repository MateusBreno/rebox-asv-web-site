...
const FormContractEdit: React.FC<IProps> = ({ contract: contractData }) => {
  ...
  const [
    enabledFieldsRateNumber,
    setEnabledFieldsRateNumber,
  ] = useState<boolean>(
    contract.rate_type !== ConfigValues.rebox.contract.rate_type.recurrent,
  );
  const [
    enabledFieldsRateAmount,
    setEnabledFieldsRateAmount,
  ] = useState<boolean>(
    contract.rate_type === ConfigValues.rebox.contract.rate_type.undefined,
  );
  const [
    enabledFieldsRateDescription,
    setEnabledFieldsRateDescription,
  ] = useState<boolean>(
    contract.rate_type === ConfigValues.rebox.contract.rate_type.undefined,
  );
  ...
  const handleUpdateFormByProduct = async (products_id: string) => {
    try {
      ...
      const [gracePeriodExpirationDate, gracePeriodExpirationHour] = date.split(
        ' ',
      );
      ...
      const { data: responseUpdateContract } = await apiRebox.put(
        `/contracts/${contract?.id}`,
        {
          ...
          discount_amount_installments: data.discount_amount_installments
            ...
            : 0,
          number_installments_with_discount: data.number_installments_with_discount
          ...
          number_calls_allowed_in_seasonality: data.number_calls_allowed_in_seasonality
            ? Number.parseInt(`${data.number_calls_allowed_in_seasonality}`, 10)
            : 0,
