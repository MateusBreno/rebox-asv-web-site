...
const FormCalledEdit: React.FC<IProps> = ({ called, refresh }) => {
  ...
  const [calledAddressesOrigin, setCalledAddressesOrigin] =
    useState<CalledAddresses>();
  const [calledAddressesDestination, setCalledAddressesDestination] =
    useState<CalledAddresses>();
  ...
  const [googleDestinationAddress, setGoogleDestinationAddress] =
  ...
  const buildInitialData = useCallback(() => {
    ...
    if (call_initiation_date) {
      const [callInitiationDate, callInitiationHour] =
    ...
    if (service_start_date) {
      const [serviceStartDate, serviceStartHour] =
        service_start_date.split(' ');
  ...
  const handleCurrentLocation = (option: string) => {
    navigator.geolocation.getCurrentPosition(async position => {
      ...
      const { country, city, neighborhood, state, street, number, zip_code } =
  ...
  const handleChangeFormByStatus = useCallback(
    (status: string) => {
      const { open, in_progress, in_attendance, done, canceled, deleted } =
