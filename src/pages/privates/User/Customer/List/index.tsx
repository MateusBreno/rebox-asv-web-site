...
const CustomerList: React.FC = () => {
  ...
  const buildTheQueryUrl = (value: number): string => {
    let url = `/users?role=${ConfigValues.rebox.user.role.client}&page=${value}&per_page=${ConfigRules.rebox.pagination.users.itemLimit}&details=all`;
      ...
      for await (const url of arrayUrls) {
        ...
        customersExported.forEach(item => {
          ...
          customersExportData.push({
            ...
            user_epharma: item.epharma_holder_id ? 'Cadastrado' : 'Aguardando',
      ...
      <ContainerGroup>
        ...
        <Content>
          ...
          <Options>
            ...
            <FormSearch ref={formSearchRef} onSubmit={handleSearchCustomers}>
              <InputText
                name="search"
          ...
          <ListUserCustomers
            ...
            loading={loading}
            showTotal={true}
            totalValue={customers?.header.total}
          />
          <Pagination>
            <ButtonDefault
              style={{
